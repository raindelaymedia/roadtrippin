"""
Girls Tripp — Dashboard Generator
Reads tracker_data_gt.json, revenue_gt.csv, and socials_gt.csv and generates
...
Usage:
    python build_gt_dashboard.py
    python build_gt_dashboard.py --output girls_tripp.html
"""
...
def extract(path):
    """Load tracker_data_gt.json → dict matching the dashboard schema.
    ...
    <div class="card">
      ...
      <div style="height:320px"><canvas id="rev-chart"></canvas></div>
        ...
        revenue_content = empty_state(
            "No revenue data yet",
            "Add rows to revenue_gt.csv to start tracking. Format: period,source,amount")
...
<div class="rev-modal-bg" id="socGhModal" onclick="if(event.target===this)srSocEditor.closeGh()">
  <div class="rev-modal">
    ...
    <div class="rev-field"><label>File path</label><input id="socGhPath" value="master/shows/road_trippin/data/girls_tripp/socials_gt.csv" spellcheck="false"></div>
...
const srSocEditor = createEditor({{
  data: SOC_EDIT_DATA, type:'soc', prefix:'soc', lsKey:'gt_soc_editor_gh',
...
def main():
    parser = argparse.ArgumentParser(description="Girls Tripp — Dashboard Generator")
    parser.add_argument("--tracker",  default=None, help="Path to tracker_data_gt.json")
    ...
    data_dir = os.path.join(script_dir, "data", "girls_tripp")

    tracker_path = args.tracker or os.path.join(data_dir, "tracker_data_gt.json")
    revenue_path = args.revenue or os.path.join(data_dir, "revenue_gt.csv")
    socials_path = args.socials or os.path.join(data_dir, "socials_gt.csv")
