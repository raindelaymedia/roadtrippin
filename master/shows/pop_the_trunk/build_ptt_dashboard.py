"""
Pop the Trunk — Dashboard Generator
Reads tracker_data_ptt.json, revenue_ptt.csv, and socials_ptt.csv and generates
...
Usage:
    python build_ptt_dashboard.py
    python build_ptt_dashboard.py --output pop_the_trunk.html
"""
...
def extract(path):
    """Load tracker_data_ptt.json → dict matching the dashboard schema.
...
def build_html(d, revenue, socials, generated_at):
    ...
    <div class="card">
      ...
      <div style="height:320px"><canvas id="rev-chart"></canvas></div>
        ...
        revenue_content = empty_state(
            "No revenue data yet",
            "Add rows to revenue_ptt.csv to start tracking. Format: period,source,amount")
    ...
    <div class="soc-grid">{platform_cards}</div>"""
        ...
        socials_content = empty_state(
            "No socials data yet",
            "Add rows to socials_ptt.csv to start tracking. Format: period,platform,metric,value")
...
<title>Pop the Trunk — Dashboard</title>
...
<style>
/* ─────────────────────────────────────────────────────────────
   POP THE TRUNK DASHBOARD — Rain Delay Media
   Brand color: #DD4B5C (gold)
   ───────────────────────────────────────────────────────────── */
:root{{
  --brand:        #DD4B5C;
  --brand-deep:   #B83A49;
  --brand-soft:   rgba(221,75,92,.08);
  --brand-tint:   rgba(221,75,92,.16);
  --bg:           #f7f8fb;
  --surface:      #ffffff;
  --surface2:     #eef1f7;
  --surface3:     #dde3ee;
...
.logo-mark{{
  ...
  box-shadow:0 2px 4px rgba(221,75,92,.25);
}}
...
<nav class="sidebar">
  <div class="sidebar-logo">
    <div class="logo-mark">PTT</div>
    <div class="logo-text">
      <div class="logo-main">Pop the Trunk</div>
...
document.addEventListener('DOMContentLoaded', function() {{
  ...
  if (M.length && document.getElementById('yt-subs-chart')) {{
    new Chart(document.getElementById('yt-subs-chart'), {{
      ...
      data: {{
        labels: M,
        datasets: [{{ label: 'Subscribers', data: subs, borderColor: '#DD4B5C', backgroundColor: 'rgba(221,75,92,.08)', fill: true, tension: .3, pointRadius: 2, borderWidth: 2 }}]
      }},
      options: {{
        responsive: true, maintainAspectRatio: false,
  ...
  if (revM.length && document.getElementById('rev-chart')) {{
    new Chart(document.getElementById('rev-chart'), {{
      ...
      data: {{
        labels: revM,
        datasets: [{{ label: 'Revenue', data: revTotals, backgroundColor: '#DD4B5C', borderRadius: 3 }}]
      }},
      options: {{
        responsive: true, maintainAspectRatio: false,
        plugins: {{ legend: {{ display: false }} }},
...
def main():
    parser = argparse.ArgumentParser(description="Pop the Trunk — Dashboard Generator")
    parser.add_argument("--tracker",  default=None, help="Path to tracker_data_ptt.json")
    parser.add_argument("--revenue",  default=None)
    parser.add_argument("--socials",  default=None)
    parser.add_argument("--output",   default="pop_the_trunk.html")
    args = parser.parse_args()

    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_dir = os.path.join(script_dir, "data")

    tracker_path = args.tracker or os.path.join(data_dir, "tracker_data_ptt.json")
    revenue_path = args.revenue or os.path.join(data_dir, "revenue_ptt.csv")
    socials_path = args.socials or os.path.join(data_dir, "socials_ptt.csv")
    out = os.path.join(script_dir, args.output)

    print("=" * 55)
    print("POP THE TRUNK — DASHBOARD GENERATOR")
    print("=" * 55)
